...
class GroupPostWidget extends StatelessWidget {
  ...
  final Group group;
  final bool deletePrivilege;


  const GroupPostWidget({Key? key, required this.description, required this.photoPath, required this.user, required this.likeCount, required this.documentPath, required this.likes, required this.postReference, required this.group, required this.deletePrivilege}) : super(key: key);
            ...
            children: [
                  ...
                  if(deletePrivilege)
                    InkWell(
                      onTap: () async{
                        ...
                        await FirebaseStorage.instance.ref('groups/${group.ref.id}/posts/$photoPath').delete();
                      },
                      child: const Padding(
              ...
              if(photoPath != null)
                Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: FutureBuilder(
                      future: FirebaseStorage.instance.ref('groups/${group.ref.id}/posts/$photoPath').getDownloadURL(),
                      builder: (context, snapshot){
                        if(!snapshot.hasData){
                          return Container(
                              color: kColorBackground,
          ...
          child: Row(
            ...
            children:  [
              ...
              TextButton(
                child: Icon(
                  Icons.comment,
                  color: commentToggle ?  kColorPrimary : Colors.grey
                  ),
                onPressed: (){
                  setState(() {
